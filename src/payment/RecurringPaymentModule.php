...
namespace turquoisedragonfly\prestashop\payment;
...
abstract class RecurringPaymentModule extends PaymentModule
{
	/**
   * Efectúa el cobro de la renovación de la suscripción indicada.
	 * @param SubscriptionModel $subscription La suscripción a renovar.
	 * @param Cart $cart El carrito a facturar.
	 * @throws SubscriptionRenewalException Si ocurrió un error al renovar la suscripción.
   */
  public function renewal($subscription, $cart)
  {
		return;
	}

	/**
	 * Valida un pedido pero no envía el correo electrónico al cliente.
	 * @see validateOrder
	 */
	public function validateOrderNoMail(
		$id_cart,
		$id_order_state,
		$amount_paid,
		$payment_method,
		...
		return $this->validateOrder(
			...
			$message, $extra_vars, $currency_special,
			$dont_touch_amount, $secure_key, $shop, false
		);
	}

	/**
	 * @param boolean $send_customer_email
	 */
		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
				...
				foreach ($order_detail_list as $key => $order_detail) {
						...
						if (isset($order->id)) {
								...
								if ($send_customer_email && $id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
